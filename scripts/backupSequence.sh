...
if screen -ls "${screenName}" | grep -q "\.${screenName}\s"; then
  ...
  sleep 1
  "${binDir}/myServer.sh" backup --config "${config_file}" --wait-server "${backupOpts}"
  sleep 1
  "${binDir}/myServer.sh" start --config "${config_file}" --wait-server --wait-backup
elif [ "${ifNeeded}" = false ]; then
  "${binDir}/myServer.sh" backup --config "${config_file}" "${backupOpts}"
fi
exit 0
